...
function EmployeeForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess(
        "Funcionário criado com sucesso. Ele já pode entrar com o acesso informado.",
      );
      setNome("");
      setEmail("");
      setPassword("");
  ...
  return (
    <main className="min-h-screen px-4 py-10 sm:px-6">
      <section className="mx-auto w-full max-w-xl rounded-3xl bg-white p-6 shadow-xl sm:p-9">
        <Link
          href="/"
          className="inline-flex items-center gap-2 text-sm font-medium text-[#765540] hover:underline"
        >
          <ArrowLeft size={17} /> Voltar ao cardápio
        </Link>
        <div className="mt-8">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#926e56]/15 text-[#765540]">
            <UserPlus />
          </div>
          <p className="mt-5 text-sm font-semibold uppercase tracking-[0.18em] text-[#926e56]">
            Administração
          </p>
          <h1 className="mt-2 text-3xl font-bold text-[#382110]">
            Criar acesso de funcionário
          </h1>
          <p className="mt-2 text-sm text-zinc-600">
            O funcionário poderá entrar imediatamente com o e-mail e a senha
            definidos aqui.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          <label className="block">
            ...
            <input
              ...
              required
              className="w-full rounded-xl border border-zinc-300 px-4 py-3 focus:border-[#926e56] focus:ring-2 focus:ring-[#926e56]/20"
